export const SUBREDDIT_CONFIGS: Record<string, any> = {
  BESalary: {
    ...
    fieldMappings: {
      ...
      officialHours: /Official hours per week:\s*(\d+(?:\.\d+)?)/,
      averageHours: /Average hours per week:\s*(\d+(?:\.\d+)?)/,
      shiftDescription: /Shift description:\s*(.+?)(?=\n|$)/,
      onCall: /On-call:\s*(.+?)(?=\n|$)/,
      vacationDays: /Vacation days per year:\s*(\d+(?:\.\d+)?)/,
      ...
      teleworkDays: /Telework days per week:\s*(\d+(?:\.\d+)?)/,

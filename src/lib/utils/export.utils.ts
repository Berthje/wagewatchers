...
export const exportToCSV = (entries: SalaryEntry[]) => {
    ...
    link.setAttribute(
        "download",
        `salary-data-${new Date().toISOString().split("T")[0]}.csv`
    );
...
export const exportToPDF = (
    ...
    const avgSalary =
        entries.reduce((sum, e) => sum + (e.grossSalary || 0), 0) /
        totalEntries;
    ...
    autoTable(doc, {
        ...
        body: [
            ...
            [
                "Median Salary",
                `€${medianSalary.toLocaleString(undefined, { maximumFractionDigits: 0 })}`,
            ],
            [
                "Countries",
                new Set(
    ...
    // Save the PDF
    doc.save(
        `salary-statistics-${new Date().toISOString().split("T")[0]}.pdf`
    );
};
 ...
 */
export function checkRateLimit(identifier: string, limit: number = 5): RateLimitResult {
  const now = new Date();
 ...
 */
export function getRemainingRequests(identifier: string, limit: number = 5): number {
  const entry = rateLimitStore.get(identifier);

...
export function useFilters(
    ...
    const [minAge, setMinAge] = useState<number | null>(
        initialFilters?.minAge || null
    );
    const [maxAge, setMaxAge] = useState<number | null>(
        initialFilters?.maxAge || null
    );
    ...
    const filteredEntries = useMemo(() => {
        ...
        if (minWorkExperience !== null) {
            filtered = filtered.filter((entry) =>
                entry.workExperience !== null && entry.workExperience >= minWorkExperience
            );
        }
        if (maxWorkExperience !== null) {
            filtered = filtered.filter((entry) =>
                entry.workExperience !== null && entry.workExperience <= maxWorkExperience
            );
        }

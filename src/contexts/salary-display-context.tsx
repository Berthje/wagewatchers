...
const SalaryDisplayContext = createContext<
    SalaryDisplayContextType | undefined
>(undefined);
...
export function SalaryDisplayProvider({
    ...
    const [preferences, setPreferences] =
        useState<SalaryDisplayPreferences>(DEFAULT_PREFERENCES);
...
export function useSalaryDisplay() {
    ...
    if (context === undefined) {
        throw new Error(
            "useSalaryDisplay must be used within a SalaryDisplayProvider"
        );
    }
    return context;
}
...
export function formatSalaryWithPreferences(
    ...
    return compact && converted >= 1000
        ? formatCompact(converted, targetCurrency)
        : formatted;
}

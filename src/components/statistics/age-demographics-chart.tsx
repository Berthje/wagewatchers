...
export function AgeDemographicsChart({ data, loading = false }: AgeDemographicsChartProps) {
  ...
  if (loading) {
    return (
      <Card className="border-border bg-card space-y-3">
        <CardHeader>
          <CardTitle className="text-foreground">{t("charts.ageDemographics.title")}</CardTitle>
          <CardDescription className="text-muted-foreground">
            {t("charts.ageDemographics.description")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="w-full h-64 md:h-96 bg-muted rounded p-4 flex items-center justify-center">
            <div className="relative">
              {/* Pie chart skeleton */}
              <div className="w-48 h-48 rounded-full bg-muted animate-pulse relative">
                {/* Pie segments */}
                <div className="absolute inset-2 rounded-full bg-muted">
                  <div
                    className="absolute top-2 left-2 w-8 h-8 bg-muted rounded-full animate-pulse"
                    style={{ animationDelay: "0.1s" }}
                  ></div>
                  <div
                    className="absolute top-8 right-4 w-6 h-6 bg-muted rounded-full animate-pulse"
                    style={{ animationDelay: "0.2s" }}
                  ></div>
                  <div
                    className="absolute bottom-6 left-6 w-10 h-10 bg-muted rounded-full animate-pulse"
                    style={{ animationDelay: "0.3s" }}
                  ></div>
                  <div
                    className="absolute bottom-4 right-8 w-7 h-7 bg-muted rounded-full animate-pulse"
              ...
              <div className="absolute -right-20 top-8 space-y-2">
                {[0, 1, 2, 3].map((i) => (
                  <div key={`age-legend-${i}`} className="flex items-center space-x-2">
                    ...
                    <div className="h-3 bg-muted rounded animate-pulse w-10 md:w-12"></div>
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <Card className="border-border bg-card space-y-3">
      ...
      <CardContent>
        <div className="w-full h-64 md:h-96 flex items-center justify-center text-muted-foreground">
